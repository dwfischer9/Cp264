...
//------------------ Task 3 ---------------------------
void init_str() {
	char s1[MAX]; // max = 20, 19 characters plus the null
	strcpy(s1, "String1");
...
//convert a word from pluarl to singular
// if word ends with 's' remove the s or 'ies' change 'ies' to 'y'
//assume word length is atleast four characters
//assume word is created using dynamic memory allocation with exact size
//after converting to singular, change the size of the word
void plural_to_singular(char* word){
	if(!word)
		return;
	int length = strlen(word);
	if(length < 4 || word[length-1])
		return;
