...
int extract_minutes(char *filename, unsigned int *minutes_array, const int size) {
	...
	int counter = 0;
	mm[2] = '\0';
	while(fgets(line,sizeof(line),in_file) && counter < size){
		mm[0] = line[3];
		mm[1] = line[4];
...
void print_record_info3(char *record) {
	...
	buffer1 = strtok(record, ":");// get rid of "record:"
	buffer1 = strtok(NULL, "-");// get year
	sscanf(buffer1, "%d", &reg_year);

	buffer1 = strtok(NULL, "-");//stores birthdate
	buffer3 = strtok(NULL, "-");//stores name
