...
int* find_match(int *array, const int size, int value, int **last) {
    ...
    if(array == NULL){
        ...
        // first = NULL; you need to return null because if you make first as NULL it will still contine this function
        return NULL;
    }
    if(size <= 0){
        printf("Error(find_match): invalid size");
        // first = NULL; // same with this you need to return NULL so it break you out of the function
        return NULL;
    }
        for(i = 0; i < size; i++){
            if( (array[i]) == value && check == 0){
                firstindex = i;
                lastindex = i;
                check = 1;
            }
            else if((array[i]) == value && check == 1){
                lastindex = i;
            }
    }
        if(check == 0)
...
int main() {
	setbuf(stdout, NULL);
	int nums[MAX] = { 22, 25, 20, 27, 22, 20, 24, 19, 28, 26, 23, 24 }, size = MAX;
